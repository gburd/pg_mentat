-- pg_mentat 1.4.0 -> 1.5.0 upgrade: append-only datom model.
--
-- 1.5.0 makes the datom log truly append-only (no in-place added flip)
-- and introduces the current-state projection (nine mentat.current_<type>
-- tables) that current-time queries read from. This upgrade:
--   1. Creates the projection tables, indexes, view, and helper functions.
--   2. Retunes the nine log tables to fillfactor=100 (append-only: the
--      reserved HOT space the old in-place flip needed is now waste).
--   3. Populates the projection from the existing log via
--      rebuild_current_projection(0). REQUIRED: without this, current-time
--      queries return nothing after the upgrade.
--
-- NOTE: pre-1.5.0 history was flip-based (superseded assertions had their
-- added flag flipped to false in place). Those rows remain as-is; the
-- projection is rebuilt by latest-tx-wins resolution, which is correct
-- against both flip-era and append-only-era history. Going forward all
-- retractions are appended, never flipped.

-- === 1. Projection tables + indexes + view + helpers ===
-- (identical to sql/24_current_projection.sql, which runs only on fresh
--  CREATE EXTENSION; replayed here for in-place upgrades.)
...
-- === 2. Retune log tables to fillfactor=100 (append-only) ===
DO $$
DECLARE t TEXT;
BEGIN
    FOREACH t IN ARRAY ARRAY[
        ...
        EXECUTE format('ALTER TABLE mentat.%I SET (fillfactor = 100)', t);
    END LOOP;
END $$;

-- === 3. Populate the projection from the existing log (REQUIRED) ===
SELECT mentat.rebuild_current_projection(0);

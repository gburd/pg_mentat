-- pg_mentat 1.5.6 -> 1.5.7 upgrade.
--
-- 1.5.7 adds entid-collision diagnostics/repair (mentat.entid_collision_report
-- / mentat.repair_entid_collisions, created by the extension's function SQL)
-- and re-asserts the safe sequence bounds. A store on 1.5.6 already had its
-- 1.5.6 migration succeed (so its sequences were within band and are now
-- correctly bounded), or is a fresh 1.5.6 install (new layout). Re-apply the
-- GREATEST(intended, head) bounding defensively so this edge is safe even if
-- the sequences advanced further since 1.5.6.
--
-- Never lowers a MAXVALUE below the live head; no id is moved.

...
mod tests {
    ...
    fn test_hi_multiple_updates_history() {
        ...
        let total = Spi::get_one::<i64>(&format!(
            "SELECT COUNT(*) FROM mentat.datoms
             WHERE e = {} AND a = (SELECT entid FROM mentat.idents WHERE ident = ':hi/val')",
            eid
        ))
        .expect("q")
        .expect("NULL");
        ...
        assert!(
            total >= 5,
            "Should have at least 5 datoms for 5 updates, got {}",
            total
        );
    }
    ...
    fn test_hi_tx_ids_increasing() {
        ...
        let txs = Spi::get_one::<String>(&format!(
            "SELECT json_agg(DISTINCT tx ORDER BY tx)::text FROM mentat.datoms WHERE e = {}",
            eid
        ));
    ...
    fn test_hi_many_retract_history() {
        ...
        let retracted = Spi::get_one::<i64>(&format!(
            "SELECT COUNT(*) FROM mentat.datoms
             ...
             AND v_text = 'remove' AND added = false",
            eid
        ))
        .expect("q")
        .expect("NULL");
    ...
    fn test_hi_entity_lifecycle() {
        ...
        let total = Spi::get_one::<i64>(&format!(
            "SELECT COUNT(*) FROM mentat.datoms WHERE e = {}",
            eid
        ))
        .expect("q")
        .expect("NULL");
    ...
    fn test_hi_string_update_history() {
        ...
        let history_count = Spi::get_one::<i64>(&format!(
            "SELECT COUNT(*) FROM mentat.datoms
             WHERE e = {} AND a = (SELECT entid FROM mentat.idents WHERE ident = ':hi/name')",
            eid
        ))
        .expect("q")
        .expect("NULL");
        ...
        assert!(
            history_count >= 5,
            "Should have at least 5 history datoms, got {}",
            history_count
        );
    }

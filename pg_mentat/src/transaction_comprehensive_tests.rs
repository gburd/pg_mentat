...
mod tests {
    ...
    fn test_tx_report_has_tempids() {
        ...
        assert!(
            j["tempids"]["e1"].as_i64().is_some(),
            "e1 tempid should be assigned"
        );
    }
    ...
    fn test_tx_report_no_tempids_for_existing() {
        ...
        let r2 = Spi::get_one::<String>(&format!(
            "SELECT mentat_transact('[[:db/add {} :tx/val 42]]'::TEXT)",
            eid
        ))
        .expect("tx")
        .expect("NULL");
    ...
    fn test_tx_add_vector_form() {
        setup();
        setup_tx_schema();
        Spi::run("SELECT mentat_transact('[[:db/add \"e\" :tx/name \"vector\"]]'::TEXT)")
            .expect("vector add");
    }
    ...
    fn test_tx_add_map_form() {
        setup();
        setup_tx_schema();
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :tx/name \"map\"}]'::TEXT)")
            .expect("map add");
    }
    ...
    fn test_tx_retract_specific_value() {
        ...
        Spi::run(&format!(
            "SELECT mentat_transact('[[:db/retract {} :tx/name \"retractme\"]]'::TEXT)",
            eid
        ))
        .expect("retract");
    ...
    fn test_tx_retract_wrong_value_is_noop() {
        ...
        let _result = Spi::get_one::<String>(&format!(
            "SELECT mentat_transact('[[:db/retract {} :tx/name \"wrong\"]]'::TEXT)",
            eid
        ));
    ...
    fn test_tx_cas_success() {
        ...
        setup_tx_schema();
        let r =
            Spi::get_one::<String>("SELECT mentat_transact('[[:db/add \"e\" :tx/val 10]]'::TEXT)")
                .expect("tx")
                .expect("NULL");
        let j: serde_json::Value = serde_json::from_str(&r).expect("parse");
        let eid = j["tempids"]["e"].as_i64().expect("eid");
        ...
        Spi::run(&format!(
            "SELECT mentat_transact('[[:db/cas {} :tx/val 10 20]]'::TEXT)",
            eid
        ))
        .expect("CAS should succeed");

        let q = Spi::get_one::<String>(&format!(
            "SELECT mentat_query('[:find ?v . :where [{} :tx/val ?v]]'::TEXT, '{{}}'::jsonb)::TEXT",
            eid
        ))
        .expect("q")
        .expect("NULL");
    ...
    fn test_tx_cas_failure_wrong_old() {
        ...
        assert!(
            raises_error(&format!(
                "SELECT mentat_transact('[[:db/cas {} :tx/val 99 20]]'::TEXT)",
                eid
            )),
    ...
    fn test_tx_cas_from_nil() {
        ...
        Spi::run(&format!(
            "SELECT mentat_transact('[[:db/cas {} :tx/val nil 42]]'::TEXT)",
            eid
        ))
    ...
    fn test_tx_cas_sequential() {
        ...
        setup_tx_schema();
        let r =
            Spi::get_one::<String>("SELECT mentat_transact('[[:db/add \"e\" :tx/val 0]]'::TEXT)")
                .expect("tx")
                .expect("NULL");
        let j: serde_json::Value = serde_json::from_str(&r).expect("parse");
        let eid = j["tempids"]["e"].as_i64().expect("eid");
        ...
        for i in 0..10 {
            Spi::run(&format!(
                "SELECT mentat_transact('[[:db/cas {} :tx/val {} {}]]'::TEXT)",
                eid,
                i,
                i + 1
            ))
    ...
    fn test_tx_upsert_does_not_create_duplicate() {
        ...
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :tx/uid \"uid-dup\" :tx/val 1}]'::TEXT)")
            .expect("create");

        for i in 2..=10 {
            Spi::run(&format!(
                "SELECT mentat_transact('[{{:db/id \"u\" :tx/uid \"uid-dup\" :tx/val {}}}]'::TEXT)",
                i
            ))
            .expect("upsert");
        }
    ...
    fn test_tx_unique_value_rejects_duplicate() {
        ...
        Spi::run("SELECT mentat_transact('[[:db/add \"e1\" :tx/code \"CODE-1\"]]'::TEXT)")
            .expect("first");
    ...
    fn test_tx_atomicity_unique_violation_rolls_back() {
        ...
        Spi::run("SELECT mentat_transact('[[:db/add \"existing\" :tx/code \"TAKEN\"]]'::TEXT)")
            .expect("setup");
    ...
    fn test_tx_sequential_50_transactions() {
        ...
        for i in 1..=50 {
            Spi::run(&format!(
                "SELECT mentat_transact('[[:db/add {} :tx/val {}]]'::TEXT)",
                eid, i
            ))
            .expect("update");
        }
    ...
    fn test_tx_idempotent_add() {
        ...
        Spi::run(&format!(
            "SELECT mentat_transact('[[:db/add {} :tx/name \"idem\"]]'::TEXT)",
            eid
        ))
    ...
    fn test_tx_ref_tempid_resolution() {
        ...
        let q = Spi::get_one::<String>(&format!(
            "SELECT mentat_query('[:find ?r . :where [{} :tx/ref ?r]]'::TEXT, '{{}}'::jsonb)::TEXT",
            child
        ))
        .expect("q")
        .expect("NULL");

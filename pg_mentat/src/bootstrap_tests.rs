...
mod tests {
    ...
    fn test_bs_idents_table_exists() {
        crate::ensure_extension_loaded();
        Spi::run("SELECT bootstrap_schema()").expect("bootstrap");
        let count = Spi::get_one::<i64>("SELECT COUNT(*) FROM mentat.idents")
            .expect("q")
            .expect("NULL");
        assert!(count > 0);
    }
    ...
    fn test_bs_transactions_table_exists() {
        crate::ensure_extension_loaded();
        Spi::run("SELECT bootstrap_schema()").expect("bootstrap");
        let count = Spi::get_one::<i64>("SELECT COUNT(*) FROM mentat.transactions")
            .expect("q")
            .expect("NULL");
        assert!(count >= 0);
    }
    ...
    fn test_bs_ident_db_value_type_exists() {
        ...
        Spi::run("SELECT bootstrap_schema()").expect("bootstrap");
        let count =
            Spi::get_one::<i64>("SELECT COUNT(*) FROM mentat.idents WHERE ident = ':db/valueType'")
                .expect("q")
                .expect("NULL");
        assert_eq!(count, 1);
    }
    ...
    fn test_bs_ident_db_cardinality_exists() {
        ...
        let count = Spi::get_one::<i64>(
            "SELECT COUNT(*) FROM mentat.idents WHERE ident = ':db/cardinality'",
        )
        .expect("q")
        .expect("NULL");
        assert_eq!(count, 1);
    ...
    fn test_bs_ident_db_unique_exists() {
        ...
        Spi::run("SELECT bootstrap_schema()").expect("bootstrap");
        let count =
            Spi::get_one::<i64>("SELECT COUNT(*) FROM mentat.idents WHERE ident = ':db/unique'")
                .expect("q")
                .expect("NULL");
        assert_eq!(count, 1);
    }
    ...
    fn test_bs_ident_db_doc_exists() {
        ...
        Spi::run("SELECT bootstrap_schema()").expect("bootstrap");
        let count =
            Spi::get_one::<i64>("SELECT COUNT(*) FROM mentat.idents WHERE ident = ':db/doc'")
                .expect("q")
                .expect("NULL");
        assert_eq!(count, 1);
    }
    ...
    fn test_bs_ident_db_tx_instant_exists() {
        ...
        Spi::run("SELECT bootstrap_schema()").expect("bootstrap");
        let count =
            Spi::get_one::<i64>("SELECT COUNT(*) FROM mentat.idents WHERE ident = ':db/txInstant'")
                .expect("q")
                .expect("NULL");
        assert_eq!(count, 1);
    }

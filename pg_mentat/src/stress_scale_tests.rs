...
mod tests {
    ...
    fn test_ss_batch_500_entities() {
        ...
        for i in 0..500 {
            ops.push(format!(
                "{{:db/id \"e{i}\" :ss/name \"ent-{i}\" :ss/val {i}}}",
                i = i
            ));
        }
        Spi::run(&format!(
    ...
    fn test_ss_define_50_attrs() {
        ...
        for i in 0..50 {
            assert!(
                result.contains(&format!("ss.gen/attr-{}", i)),
                "attr-{} missing",
                i
            );
        }
    ...
    fn test_ss_batch_retract_100_entities() {
        ...
        for i in 0..100 {
            ops.push(format!(
                "{{:db/id \"e{i}\" :ss/name \"doomed-{i}\" :ss/val {i}}}",
                i = i
            ));
        }
        let r = Spi::get_one::<String>(&format!(
    ...
    fn test_ss_create_update_query_cycle() {
        ...
        for i in 0..50 {
            ops.push(format!(
                "{{:db/id \"e{i}\" :ss/name \"cycle-{i}\" :ss/val 0}}",
                i = i
            ));
        }
        let r = Spi::get_one::<String>(&format!(
        ...
        let v: serde_json::Value = serde_json::from_str(&q).expect("parse");
        assert!(
            v["result"].as_array().expect("arr").len() > 0,
            "Should find some values > 40"
        );
    }
}

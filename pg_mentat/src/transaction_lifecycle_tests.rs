...
mod tests {
    ...
    fn test_tl_report_valid_json() {
        ...
        for i in 0..10 {
            let r = Spi::get_one::<String>(&format!(
                "SELECT mentat_transact('[[:db/add \"e{}\" :tl/val {}]]'::TEXT)",
                i, i
            ))
            .expect("tx")
            .expect("NULL");
    ...
    fn test_tl_report_existing_entity_no_tempid() {
        ...
        let r2 = Spi::get_one::<String>(&format!(
            "SELECT mentat_transact('[[:db/add {} :tl/val 42]]'::TEXT)",
            eid
        ))
        .expect("tx")
        .expect("NULL");
    ...
    fn test_tl_tx_id_unique_after_20() {
        ...
        for i in 0..20 {
            let r = Spi::get_one::<String>(&format!(
                "SELECT mentat_transact('[[:db/add \"e{}\" :tl/name \"n-{}\"]]'::TEXT)",
                i, i
            ))
            .expect("tx")
            .expect("NULL");
    ...
    fn test_tl_create_read_update_delete() {
        ...
        let q = Spi::get_one::<String>(&format!(
            "SELECT mentat_query('[:find ?v . :where [{} :tl/val ?v]]'::TEXT, '{{}}'::jsonb)::TEXT",
            eid
        ))
        .expect("q")
        .expect("NULL");
        ...
        // Update
        Spi::run(&format!(
            "SELECT mentat_transact('[[:db/add {} :tl/val 200]]'::TEXT)",
            eid
        ))
        .expect("update");
        // Read again
    ...
    fn test_tl_workflow_5_entities_crud() {
        ...
        for (i, &eid) in ids.iter().enumerate() {
            Spi::run(&format!(
                "SELECT mentat_transact('[[:db/add {} :tl/val {}]]'::TEXT)",
                eid,
                (i + 1) * 100
            ))
            .expect("update");
    ...
    fn test_tl_status_machine_workflow() {
        ...
        for state in &states {
            Spi::run(&format!(
                "SELECT mentat_transact('[[:db/add {} :tl/status {}]]'::TEXT)",
                eid, state
            ))
            .expect("transition");
        }
    ...
    fn test_tl_tag_accumulation_workflow() {
        ...
        for i in 0..10 {
            Spi::run(&format!(
                "SELECT mentat_transact('[[:db/add {} :tl/tags \"tag-{}\"]]'::TEXT)",
                eid, i
            ))
            .expect("add tag");
        }
    ...
    fn test_tl_retract_and_readd() {
        ...
        // Retract the value
        Spi::run(&format!(
            "SELECT mentat_transact('[[:db/retract {} :tl/val 42]]'::TEXT)",
            eid
        ))
        .expect("retract");
        // Re-add a different value
        Spi::run(&format!(
            "SELECT mentat_transact('[[:db/add {} :tl/val 99]]'::TEXT)",
            eid
        ))
        .expect("readd");
        let q = Spi::get_one::<String>(&format!(
    ...
    fn test_tl_schema_then_100_data_txs() {
        ...
        for i in 0..100 {
            Spi::run(&format!(
                "SELECT mentat_transact('[[:db/add \"e{}\" :tl/name \"n{}\" :tl/val {}]]'::TEXT)",
                i, i, i
            ))
            .expect("tx");
        }
    ...
    fn test_tl_rapid_fire_same_entity_50() {
        ...
        setup_tl_schema();
        let r =
            Spi::get_one::<String>("SELECT mentat_transact('[[:db/add \"e\" :tl/val 0]]'::TEXT)")
                .expect("tx")
                .expect("NULL");
        let j: serde_json::Value = serde_json::from_str(&r).expect("parse");
        let eid = j["tempids"]["e"].as_i64().expect("eid");
    ...
    fn test_tl_create_and_link_across_txs() {
        ...
        for i in 0..5 {
            let r = Spi::get_one::<String>(&format!(
                "SELECT mentat_transact('[[:db/add \"e{}\" :tl/name \"node-{}\"]]'::TEXT)",
                i, i
            ))
            .expect("tx")
            .expect("NULL");
        ...
        for i in 1..5 {
            Spi::run(&format!(
                "SELECT mentat_transact('[[:db/add {} :tl/ref {}]]'::TEXT)",
                eids[i],
                eids[i - 1]
            ))
            .expect("link");
    ...
    fn test_tl_bulk_upsert_then_verify() {
        ...
        for i in 0..20 {
            Spi::run(&format!(
                "SELECT mentat_transact('[{{:tl/uid \"user-{}\" :tl/val {}}}]'::TEXT)",
                i,
                (i + 1) * 100
            ))
            .expect("update");

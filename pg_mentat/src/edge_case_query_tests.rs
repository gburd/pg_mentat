...
mod tests {
    ...
    fn test_eq_nonexistent_attr_in_query() {
        setup();
        setup_eq_schema();
        Spi::run("SELECT mentat_transact('[[:db/add \"e\" :eq/name \"test\"]]'::TEXT)")
            .expect("data");
    ...
    fn test_eq_join_shared_entity() {
        setup();
        setup_eq_schema();
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :eq/name \"Alice\" :eq/val 42}]'::TEXT)")
            .expect("data");
        let q = Spi::get_one::<String>(
            "SELECT mentat_query('[:find ?n ?v :where [?e :eq/name ?n] [?e :eq/val ?v]]'::TEXT, '{}'::jsonb)::TEXT",
        ).expect("q").expect("NULL");
    ...
    fn test_eq_join_self_ref() {
        ...
        Spi::run(&format!(
            "SELECT mentat_transact('[[:db/add {} :eq/ref {}]]'::TEXT)",
            eid, eid
        ))
        .expect("self ref");
    ...
    fn test_eq_many_empty_collection() {
        setup();
        setup_eq_schema();
        Spi::run("SELECT mentat_transact('[[:db/add \"e\" :eq/name \"no-tags\"]]'::TEXT)")
            .expect("data");
        let q = Spi::get_one::<String>(
    ...
    fn test_eq_many_after_retract() {
        ...
        let eid = j["tempids"]["e"].as_i64().expect("eid");
        Spi::run(&format!(
            "SELECT mentat_transact('[[:db/retract {} :eq/tags \"remove\"]]'::TEXT)",
            eid
        ))
        .expect("retract");
        let q = Spi::get_one::<String>(&format!(
    ...
    fn test_eq_query_reflects_add() {
        setup();
        setup_eq_schema();
        Spi::run("SELECT mentat_transact('[[:db/add \"e\" :eq/name \"before\"]]'::TEXT)")
            .expect("data");
        let q1 = Spi::get_one::<String>(
        ...
        Spi::run("SELECT mentat_transact('[[:db/add \"e2\" :eq/name \"after\"]]'::TEXT)")
            .expect("data");
        let q2 = Spi::get_one::<String>(
            "SELECT mentat_query('[:find [?n ...] :where [_ :eq/name ?n]]'::TEXT, '{}'::jsonb)::TEXT",
        ).expect("q").expect("NULL");
    ...
    fn test_eq_query_reflects_retract() {
        ...
        let eid = j["tempids"]["e"].as_i64().expect("eid");
        Spi::run(&format!(
            "SELECT mentat_transact('[[:db/retract {} :eq/name \"doomed\"]]'::TEXT)",
            eid
        ))
        .expect("retract");
        let q = Spi::get_one::<String>(
    ...
    fn test_eq_query_reflects_replace() {
        ...
        setup_eq_schema();
        let r =
            Spi::get_one::<String>("SELECT mentat_transact('[[:db/add \"e\" :eq/val 10]]'::TEXT)")
                .expect("tx")
                .expect("NULL");
        let j: serde_json::Value = serde_json::from_str(&r).expect("parse");
        let eid = j["tempids"]["e"].as_i64().expect("eid");
        Spi::run(&format!(
            "SELECT mentat_transact('[[:db/add {} :eq/val 20]]'::TEXT)",
            eid
        ))
        .expect("replace");
        let q = Spi::get_one::<String>(&format!(
            "SELECT mentat_query('[:find ?v . :where [{} :eq/val ?v]]'::TEXT, '{{}}'::jsonb)::TEXT",
            eid
        ))
        .expect("q")
        .expect("NULL");
    ...
    fn test_eq_query_consistent_after_10_mutations() {
        ...
        for i in 1..=10 {
            Spi::run(&format!(
                "SELECT mentat_transact('[[:db/add {} :eq/val {}]]'::TEXT)",
                eid, i
            ))
            .expect("update");
        }

...
mod entity_lifecycle_tests {
    ...
    fn test_el_create_two_attrs() {
        ...
        let q = Spi::get_one::<String>(&format!(
            "SELECT mentat_query('[:find ?v . :where [{} :el/val ?v]]'::TEXT, '{{}}'::jsonb)::TEXT", eid
    ...
    fn test_el_create_batch_10() {
        ...
        let mut ops = vec![];
        for i in 0..10 {
            ops.push(format!("{{:db/id \"e{}\" :el/name \"item-{}\" :el/val {}}}", i, i, i));
        }
    ...
    fn test_el_create_batch_50() {
        ...
        let mut ops = vec![];
        for i in 0..50 {
            ops.push(format!("{{:db/id \"e{}\" :el/name \"item-{}\" :el/val {} :el/flag {}}}", i, i, i, if i % 2 == 0 { "true" } else { "false" }));
        }
    ...
    fn test_el_create_via_upsert() {
        setup(); setup_el_schema();
        Spi::run("SELECT mentat_transact('[{:el/uid \"bob\" :el/val 100}]'::TEXT)").expect("create");
        let q = Spi::get_one::<String>(
    ...
    fn test_el_create_sequential_10() {
        ...
        for i in 0..10 {
            Spi::run(&format!(
                "SELECT mentat_transact('[[:db/add \"e{}\" :el/name \"seq-{}\"]]'::TEXT)", i, i
            )).expect("tx");
        }
        let q = Spi::get_one::<String>(
            "SELECT mentat_query('[:find [?n ...] :where [_ :el/name ?n]]'::TEXT, '{}'::jsonb)::TEXT",
    ...
    fn test_el_create_map_and_vector_forms() {
        setup(); setup_el_schema();
        // Map form
        Spi::run("SELECT mentat_transact('[{:db/id \"e1\" :el/name \"map-form\" :el/val 1}]'::TEXT)").expect("map");
        // Vector form
    ...
    fn test_el_update_string() {
        ...
        let j: serde_json::Value = serde_json::from_str(&r).expect("parse");
        let eid = j["tempids"]["e"].as_i64().expect("eid");
        Spi::run(&format!("SELECT mentat_transact('[[:db/add {} :el/name \"after\"]]'::TEXT)", eid)).expect("update");
        let q = Spi::get_one::<String>(&format!(
    ...
    fn test_el_update_long() {
        setup(); setup_el_schema();
        let r = Spi::get_one::<String>(
            "SELECT mentat_transact('[[:db/add \"e\" :el/val 10]]'::TEXT)",
        ).expect("tx").expect("NULL");
        let j: serde_json::Value = serde_json::from_str(&r).expect("parse");
        let eid = j["tempids"]["e"].as_i64().expect("eid");
        Spi::run(&format!("SELECT mentat_transact('[[:db/add {} :el/val 99]]'::TEXT)", eid)).expect("update");
        let q = Spi::get_one::<String>(&format!(
            "SELECT mentat_query('[:find ?v . :where [{} :el/val ?v]]'::TEXT, '{{}}'::jsonb)::TEXT", eid
        )).expect("q").expect("NULL");
        let v: serde_json::Value = serde_json::from_str(&q).expect("parse");
    ...
    fn test_el_update_boolean() {
        ...
        let j: serde_json::Value = serde_json::from_str(&r).expect("parse");
        let eid = j["tempids"]["e"].as_i64().expect("eid");
        Spi::run(&format!("SELECT mentat_transact('[[:db/add {} :el/flag false]]'::TEXT)", eid)).expect("update");
        let q = Spi::get_one::<String>(&format!(
    ...
    fn test_el_update_keyword() {
        ...
        let j: serde_json::Value = serde_json::from_str(&r).expect("parse");
        let eid = j["tempids"]["e"].as_i64().expect("eid");
        Spi::run(&format!("SELECT mentat_transact('[[:db/add {} :el/status :published]]'::TEXT)", eid)).expect("update");
        let q = Spi::get_one::<String>(&format!(
    ...
    fn test_el_add_tag_to_existing() {
        ...
        let j: serde_json::Value = serde_json::from_str(&r).expect("parse");
        let eid = j["tempids"]["e"].as_i64().expect("eid");
        Spi::run(&format!("SELECT mentat_transact('[[:db/add {} :el/tags \"new-tag\"]]'::TEXT)", eid)).expect("add tag");
        let q = Spi::get_one::<String>(&format!(
    ...
    fn test_el_update_ref_target() {
        ...
        Spi::run(&format!("SELECT mentat_transact('[[:db/add {} :el/ref {}]]'::TEXT)", src, t2)).expect("update ref");
        let q = Spi::get_one::<String>(&format!(
    ...
    fn test_el_update_via_upsert() {
        setup(); setup_el_schema();
        Spi::run("SELECT mentat_transact('[{:el/uid \"alice\" :el/val 100}]'::TEXT)").expect("create");
        Spi::run("SELECT mentat_transact('[{:el/uid \"alice\" :el/val 200}]'::TEXT)").expect("upsert");
        let q = Spi::get_one::<String>(
    ...
    fn test_el_update_20_sequential() {
        setup(); setup_el_schema();
        let r = Spi::get_one::<String>(
            "SELECT mentat_transact('[[:db/add \"e\" :el/val 0]]'::TEXT)",
        ).expect("tx").expect("NULL");
        let j: serde_json::Value = serde_json::from_str(&r).expect("parse");
        let eid = j["tempids"]["e"].as_i64().expect("eid");
        for i in 1..=20 {
            Spi::run(&format!("SELECT mentat_transact('[[:db/add {} :el/val {}]]'::TEXT)", eid, i)).expect("update");
        }
        let q = Spi::get_one::<String>(&format!(
            "SELECT mentat_query('[:find ?v . :where [{} :el/val ?v]]'::TEXT, '{{}}'::jsonb)::TEXT", eid
        )).expect("q").expect("NULL");
    ...
    fn test_el_batch_update_20_entities() {
        ...
        for i in 0..20 {
            create_ops.push(format!("{{:db/id \"e{}\" :el/name \"item-{}\" :el/val 0}}", i, i));
        }
        let r = Spi::get_one::<String>(&format!(
            "SELECT mentat_transact('[{}]'::TEXT)", create_ops.join("\n")
        )).expect("tx").expect("NULL");
        let j: serde_json::Value = serde_json::from_str(&r).expect("parse");
    ...
    fn test_el_retract_single_attr() {
        ...
        let j: serde_json::Value = serde_json::from_str(&r).expect("parse");
        let eid = j["tempids"]["e"].as_i64().expect("eid");
        Spi::run(&format!("SELECT mentat_transact('[[:db/retract {} :el/val 42]]'::TEXT)", eid)).expect("retract");
        let q = Spi::get_one::<String>(&format!(
            "SELECT mentat_query('[:find ?v . :where [{} :el/val ?v]]'::TEXT, '{{}}'::jsonb)::TEXT", eid
        )).expect("q").expect("NULL");
        let v: serde_json::Value = serde_json::from_str(&q).expect("parse");
    ...
    fn test_el_retract_many_value() {
        ...
        let j: serde_json::Value = serde_json::from_str(&r).expect("parse");
        let eid = j["tempids"]["e"].as_i64().expect("eid");
        Spi::run(&format!("SELECT mentat_transact('[[:db/retract {} :el/tags \"b\"]]'::TEXT)", eid)).expect("retract");
        let q = Spi::get_one::<String>(&format!(
            "SELECT mentat_query('[:find [?t ...] :where [{} :el/tags ?t]]'::TEXT, '{{}}'::jsonb)::TEXT", eid
        )).expect("q").expect("NULL");
        let v: serde_json::Value = serde_json::from_str(&q).expect("parse");
    ...
    fn test_el_retract_doesnt_affect_others() {
        ...
        Spi::run(&format!("SELECT mentat_transact('[[:db/retractEntity {}]]'::TEXT)", e2)).expect("retract");
        let q = Spi::get_one::<String>(
            "SELECT mentat_query('[:find [?n ...] :where [_ :el/name ?n]]'::TEXT, '{}'::jsonb)::TEXT",
        ).expect("q").expect("NULL");
        let v: serde_json::Value = serde_json::from_str(&q).expect("parse");
    ...
    fn test_el_retract_then_readd() {
        setup(); setup_el_schema();
        let r = Spi::get_one::<String>(
            "SELECT mentat_transact('[[:db/add \"e\" :el/val 42]]'::TEXT)",
        ).expect("tx").expect("NULL");
        let j: serde_json::Value = serde_json::from_str(&r).expect("parse");
        let eid = j["tempids"]["e"].as_i64().expect("eid");
        Spi::run(&format!("SELECT mentat_transact('[[:db/retract {} :el/val 42]]'::TEXT)", eid)).expect("retract");
        Spi::run(&format!("SELECT mentat_transact('[[:db/add {} :el/val 99]]'::TEXT)", eid)).expect("readd");
        let q = Spi::get_one::<String>(&format!(
            "SELECT mentat_query('[:find ?v . :where [{} :el/val ?v]]'::TEXT, '{{}}'::jsonb)::TEXT", eid
        )).expect("q").expect("NULL");
        let v: serde_json::Value = serde_json::from_str(&q).expect("parse");
    ...
    fn test_el_retract_ref() {
        ...
        Spi::run(&format!("SELECT mentat_transact('[[:db/retract {} :el/ref {}]]'::TEXT)", child, parent)).expect("retract");
        let q = Spi::get_one::<String>(&format!(
            "SELECT mentat_query('[:find ?p . :where [{} :el/ref ?p]]'::TEXT, '{{}}'::jsonb)::TEXT", child
        )).expect("q").expect("NULL");
        let v: serde_json::Value = serde_json::from_str(&q).expect("parse");
        assert!(v["result"].is_null());
    }

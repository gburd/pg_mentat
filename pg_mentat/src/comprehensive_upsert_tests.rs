...
mod tests {
    ...
    fn test_cu_second_upsert_updates() {
        setup(); setup_cu_schema();
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :cu/uid \"U2\" :cu/name \"v1\"}]'::TEXT)").expect("c1");
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :cu/uid \"U2\" :cu/name \"v2\"}]'::TEXT)").expect("c2");
    ...
    fn test_cu_upsert_preserves_unmentioned() {
        ...
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :cu/uid \"U3\" :cu/name \"updated\"}]'::TEXT)").expect("u");
        let q = Spi::get_one::<String>(
    ...
    fn test_cu_upsert_10x_same() {
        ...
        for i in 0..10 {
            Spi::run(&format!(
                "SELECT mentat_transact('[{{:db/id \"e\" :cu/uid \"U4\" :cu/val {}}}]'::TEXT)", i
            )).expect("upsert");
        }
    ...
    fn test_cu_upsert_with_bool() {
        setup(); setup_cu_schema();
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :cu/uid \"U5\" :cu/flag true}]'::TEXT)").expect("c");
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :cu/uid \"U5\" :cu/flag false}]'::TEXT)").expect("u");
        let q = Spi::get_one::<String>(
    ...
    fn test_cu_upsert_with_keyword() {
        setup(); setup_cu_schema();
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :cu/uid \"U6\" :cu/kw :draft}]'::TEXT)").expect("c");
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :cu/uid \"U6\" :cu/kw :published}]'::TEXT)").expect("u");
        let q = Spi::get_one::<String>(
    ...
    fn test_cu_upsert_with_double() {
        setup(); setup_cu_schema();
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :cu/uid \"U7\" :cu/dbl 1.0}]'::TEXT)").expect("c");
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :cu/uid \"U7\" :cu/dbl 9.99}]'::TEXT)").expect("u");
        let q = Spi::get_one::<String>(
    ...
    fn test_cu_upsert_adds_to_many() {
        setup(); setup_cu_schema();
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :cu/uid \"UM1\" :cu/tags \"tag1\"}]'::TEXT)").expect("c");
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :cu/uid \"UM1\" :cu/tags \"tag2\"}]'::TEXT)").expect("u");
        let q = Spi::get_one::<String>(
    ...
    fn test_cu_upsert_many_idempotent() {
        setup(); setup_cu_schema();
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :cu/uid \"UM2\" :cu/tags \"dup\"}]'::TEXT)").expect("c");
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :cu/uid \"UM2\" :cu/tags \"dup\"}]'::TEXT)").expect("u");
        let q = Spi::get_one::<String>(
    ...
    fn test_cu_upsert_sets_ref() {
        ...
        Spi::run(&format!(
            "SELECT mentat_transact('[{{:db/id \"e\" :cu/uid \"UR1\" :cu/ref {}}}]'::TEXT)", target
        )).expect("u");
        let q = Spi::get_one::<String>(
    ...
    fn test_cu_upsert_replaces_ref() {
        ...
        Spi::run(&format!(
            "SELECT mentat_transact('[{{:db/id \"e\" :cu/uid \"UR2\" :cu/ref {}}}]'::TEXT)", t2
        )).expect("u");
        let q = Spi::get_one::<String>(
    ...
    fn test_cu_upsert_by_uid() {
        ...
        Spi::run("SELECT mentat_transact('[{:db/id \"e\" :cu/uid \"TU2\" :cu/name \"Bobby\"}]'::TEXT)").expect("u via uid");
        let q = Spi::get_one::<String>(
    ...
    fn test_cu_batch_upsert_10_new() {
        ...
        let mut ops = Vec::new();
        for i in 0..10 {
            ops.push(format!("{{:db/id \"e{i}\" :cu/uid \"BU-{i}\" :cu/name \"ent-{i}\" :cu/val {i}}}", i = i));
        }
    ...
    fn test_cu_batch_upsert_10_update() {
        ...
        let mut ops = Vec::new();
        for i in 0..10 {
            ops.push(format!("{{:db/id \"e{i}\" :cu/uid \"BU2-{i}\" :cu/val 0}}", i = i));
        }
        Spi::run(&format!("SELECT mentat_transact('[{}]'::TEXT)", ops.join("\n"))).expect("create");
        // Update
        let mut updates = Vec::new();
        for i in 0..10 {
            updates.push(format!("{{:db/id \"e{i}\" :cu/uid \"BU2-{i}\" :cu/val {v}}}", i = i, v = (i + 1) * 100));
        }
